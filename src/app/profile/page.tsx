import UpdateButton from "@/components/utils/UpdateButton"
import { formatISODate, updateUser, wixClientServer } from "@/lib"
...
const ProfilePage = async () => {
  ...
  if (!user?.member?.contactId) {
    ;<div>Debes de iniciar sesion</div>
  }

  const orderResponse = await wixClient.orders.searchOrders({
    search: {
      filter: {
        "buyerInfo.contactId": { $eq: user?.member?.contactId },
      },
    },
  })
        ...
        <div className="mt-12 flex flex-col">
          {orderResponse.orders.map((order) => (
            ...
            >
              ...
              {order._createdDate && (
                <span className="w-1/4">{formatISODate(order._createdDate)}</span>
              )}
